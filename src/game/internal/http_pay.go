...
func handleEdyWXPay(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case "GET":
		total_fee := r.URL.Query().Get("total_fee")
		account_id := r.URL.Query().Get("account_id")
		...
		w.Header().Set("Access-Control-Allow-Origin", "*")
		fmt.Fprintf(w, "%s", data)
		startEdyWXPayOrder(p["out_trade_no"], accountID, totalFee, nil)
	}
}

func handleEdyAliPay(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case "GET":
		total_amount := r.URL.Query().Get("total_amount")
		account_id := r.URL.Query().Get("account_id")
		...
		totalAmount, _ := strconv.ParseFloat(total_amount, 64)
		accountID, _ := strconv.Atoi(account_id)
		startEdyAliPayOrder(outTradeNo, accountID, totalAmount, nil)
	}
}